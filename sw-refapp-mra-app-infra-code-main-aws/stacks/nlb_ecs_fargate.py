...
class NLBECSFargateEndpointService(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, resource_config,
        ...
        acm = cert.Certificate(
            ...
            domain_name=f"service1.{config['service_name']}.{config['cloud_hosted_domain']}",
        ...
        logs.CfnSubscriptionFilter(
            ...
            filter_pattern="{ $.action = CREATE || $.actions = UPDATE || $.action = DELETE || $.action = FETCH }",
        ...
        managed_instance_role = iam.Role(
            ...
            role_name=f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-managed-instance-role-{config['resource_suffix']}"
        )
        ...
        ecs_task_role = iam.Role(
            ...
            role_name=
            f"{config['resource_prefix']}-{config['service_name']}-{config['service_name']}-ecs-task-execution-role-{config['resource_suffix']}"
        )
        ...
        task_definition.add_container(
            ...
            command=["-o", "-n", config['deployment_region_1']],
            cpu=32
        )
        ...
        container_definition = ecs.ContainerDefinition(
            ...
            cpu=1024,  # 1vCPU
            environment={
                **self.container_env("ecs.env", config['deployment_region_1']),
        ...
        ecs_sg.add_ingress_rule(ec2.Peer.ipv4(vpc.vpc_cidr_block),
                                ec2.Port.tcp(443))
        ecs_sg.add_ingress_rule(ec2.Peer.ipv4('192.168.1.0/24'), ec2.Port.tcp(443))
