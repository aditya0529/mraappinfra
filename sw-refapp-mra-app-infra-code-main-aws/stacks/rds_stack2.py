...
This stack will be triggered based on the global database option in "resource.config" to create the secondary regional Cluster.
"""
...
class RdsStack2(Stack):
    def __init__(self, scope: Construct, construct_id: str, resource_config, cluster=None, **kwargs) -> None:
        ...
        database_subnet_group_name=f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-db-subnet-group-{config['app_name']}-{config['deployment_region_2']}-{config['resource_suffix']}"
        database_subnet_group = rds.CfnDBSubnetGroup(
            self,
            database_subnet_group_name,
            db_subnet_group_description="Database Subnet Group",
            subnet_ids=[f"{config['subnet_id_2_1']}",f"{config['subnet_id_2_2']}"],
        ...
        rds_sg = ec2.SecurityGroup(
            ...
            "rds-sg",
            security_group_name=
            f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-rds-{config['resource_suffix']}",
            description=
        ...
        #Cluster Name.
        database_cl_name = f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-rds-cl2-{config['resource_suffix']}"

        key_arn=f"arn:aws:kms:{config['deployment_region_2']}:{config['workload_account']}:key/{config['mrk_id']}"

        #Postgresql Cluster custom parameter group.
        cluster_pg_name=f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-cl2-cpg-{config['resource_suffix']}"
        ...
        instance_pg_name=f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-rds-ipg2-{config['resource_suffix']}"
        ...
        rds_em_role_name=f"{config['resource_prefix']}-{config['service_name']}-{config['app_env']}-rds-em-role2-{config['resource_suffix']}"
        ...
        if config.get('database_instance_type') == "serverless_v2":
            if ((config.get('database_engine') == "aurora_mysql") and (config.get('database_cluster_type') == "Regional")):
                ...
                # cluster1= rds.CfnDBCluster(self, "temp-cl2",db_cluster_identifier="temp-cl2",
                #                           engine="aurora-postgresql",
                #                           engine_version="16.1",
                #                           # master_username="postgres",
                #                           # master_user_password="TempPassword",
